...
class TrainDUTSv3(torch.utils.data.Dataset):
    def __init__(self, root, clip_n):
        ...
        self.flow_list = sorted(glob(os.path.join(flow_dir, '*')))
    ...
    def __getitem__(self, idx):
        ...
        # select one from multiple flows
        flows = sorted(os.listdir(self.flow_list[frame_id]))
        target = random.choice(flows)
        flow = Image.open(self.flow_list[frame_id] + '/' + target).convert('RGB')
